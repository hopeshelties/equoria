...
describe('TraitDisplay Component', () => {
  ...
  describe('Rendering', () => {
    it('renders correctly with all trait types', () => {
      const { getByText, getByLabelText } = render(<TraitDisplay {...defaultProps} />);
      
      // Check header
      expect(getByText("Thunder's Traits")).toBeTruthy();
      expect(getByText('Epigenetic characteristics that shape behavior')).toBeTruthy();
    ...
    it('renders empty state when no traits provided', () => {
      ...
      expect(getByText('Traits will be revealed as you interact with your horse')).toBeTruthy();
    });
    ...
    it('does not render sections for empty trait arrays', () => {
      const { queryByText } = render(
        <TraitDisplay traits={{ positive: [], negative: ['nervous'], hidden: [] }} />
      );
  ...
  describe('Modal Functionality', () => {
    it('opens modal when trait is pressed', async () => {
      ...
      await waitFor(() => {
        expect(getByText('Resilient')).toBeTruthy();
        expect(getByText('This horse recovers quickly from stress and setbacks. Resilient horses bounce back faster from training challenges and maintain better emotional stability.')).toBeTruthy();
        expect(getByText('Faster stress recovery, improved training consistency')).toBeTruthy();
      });
    });

    it('closes modal when close button is pressed', async () => {
      const { getByText, getByLabelText, queryByText } = render(<TraitDisplay {...defaultProps} />);
      ...
      await waitFor(() => {
        expect(queryByText('This horse recovers quickly from stress and setbacks. Resilient horses bounce back faster from training challenges and maintain better emotional stability.')).toBeNull();
      });
    });
    ...
    it('displays correct content for negative traits', async () => {
      ...
      await waitFor(() => {
        expect(getByText('Nervous')).toBeTruthy();
        expect(getByText('Prone to anxiety and stress in challenging situations. Nervous horses require more careful handling and patience during training.')).toBeTruthy();
        expect(getByText('Increased stress sensitivity, requires gentle approach')).toBeTruthy();
      });
    });
  });
  ...
  describe('Callback Functionality', () => {
    it('calls onTraitPress callback when trait is pressed', async () => {
      ...
      await waitFor(() => {
        expect(mockCallback).toHaveBeenCalledWith('resilient', expect.objectContaining({
          name: 'Resilient',
          type: 'positive',
  ...
  describe('Accessibility', () => {
    ...
    it('has proper accessibility hints for interactive elements', () => {
      ...
      const resilientBadge = getByLabelText('Resilient trait - positive');
      expect(resilientBadge.props.accessibilityHint).toBe('Tap to view detailed description');
      
      const hiddenTrait = getByLabelText('Hidden trait - not yet discovered');
      expect(hiddenTrait.props.accessibilityHint).toBe('This trait will be revealed as you learn more about your horse');
    });
  ...
  describe('Edge Cases', () => {
    it('handles traits with underscores in names', () => {
      ...
      const { getByText } = render(<TraitDisplay traits={traitsWithUnderscores} />);
    ...
    it('handles missing trait arrays', () => {
      ...
      const { getByText, queryByText } = render(<TraitDisplay traits={incompleteTraits} />);
