...
export function useGroomManagement(playerId) {
   ...
   */
  const assignGroom = useCallback(async (foalId, groomId, options = {}) => {
    try {
   ...
   */
  const ensureDefaultAssignment = useCallback(async (foalId) => {
    try {
      setGroomState(prev => ({ ...prev, error: null }));
   ...
   * Hire a new groom
   */
  const hireGroom = useCallback(async (groomData) => {
    try {
      setGroomState(prev => ({ ...prev, error: null }));
   ...
   */
  const getActiveAssignment = useCallback((foalId) => {
   ...
   * Get groom by ID
   */
  const getGroomById = useCallback((groomId) => {
...
export function useDailyCareAutomation(foalIds = []) {
  ...
  const runDailyCare = useCallback(async (options = {}) => {
    try {
