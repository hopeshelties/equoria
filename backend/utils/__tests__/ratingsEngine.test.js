...
describe('ratingsEngine', () => {
    ...
    describe('generateStoreHorseRatings', () => {
        ...
        const conformationAttributes = ['head', 'neck', 'shoulders', 'back', 'hindquarters', 'legs', 'hooves'];
        const gaitAttrsNoGaiting = ['walk', 'trot', 'canter', 'gallop'];
        const fullGaitAttrs = [...gaitAttrsNoGaiting, 'gaiting'];

        const createProfile = (isGaited, includeConformation = true, includeGaits = true, includeGaitingProfile = true) => {
            const profile = { is_gaited_breed: isGaited };
            if (includeConformation) {
                profile.conformation = {};
                conformationAttributes.forEach(attr => profile.conformation[attr] = { mean: 50, std_dev: 10 });
            }
            if (includeGaits) {
                profile.gaits = {};
                gaitAttrsNoGaiting.forEach(attr => profile.gaits[attr] = { mean: 50, std_dev: 10 });
        ...
        it('should use injected mock function for ratings (non-gaited)', () => {
            ...
            const { conformationRatings, gaitRatings } = ratingsEngine.generateStoreHorseRatings(
                profile, 
                { generateAttributeScore: mockedGenerateAttributeScoreFunc }
            );
            conformationAttributes.forEach(attr => expect(conformationRatings[attr]).toBe(MOCKED_SCORE));
            gaitAttrsNoGaiting.forEach(attr => expect(gaitRatings[attr]).toBe(MOCKED_SCORE));
            expect(gaitRatings.gaiting).toBeNull();
            expect(mockedGenerateAttributeScoreFunc).toHaveBeenCalledTimes(conformationAttributes.length + gaitAttrsNoGaiting.length);
        });
        ...
        it('should use injected mock function for ratings (gaited)', () => {
            ...
            conformationAttributes.forEach(attr => expect(conformationRatings[attr]).toBe(MOCKED_SCORE));
            fullGaitAttrs.forEach(attr => expect(gaitRatings[attr]).toBe(MOCKED_SCORE));
            expect(mockedGenerateAttributeScoreFunc).toHaveBeenCalledTimes(conformationAttributes.length + fullGaitAttrs.length);
        });
        ...
        it('should return SUT defaults and log error if breedRatingProfiles is undefined (mock not called)', () => {
            ...
            conformationAttributes.forEach(attr => expect(conformationRatings[attr]).toBe(DEFAULT_SCORE_FROM_SUT));
            gaitAttrsNoGaiting.forEach(attr => expect(gaitRatings[attr]).toBe(DEFAULT_SCORE_FROM_SUT));
            expect(gaitRatings.gaiting).toBeNull();
            expect(mockedGenerateAttributeScoreFunc).not.toHaveBeenCalled();
            expect(consoleErrorSpy).toHaveBeenCalledWith(expect.stringContaining('breedRatingProfiles is undefined'));
        });
        ...
        it('should use SUT defaults for conformation (logs warn), injected mock for gaits', () => {
            ...
            gaitAttrsNoGaiting.forEach(attr => expect(gaitRatings[attr]).toBe(MOCKED_SCORE));
            expect(gaitRatings.gaiting).toBeNull();
            expect(mockedGenerateAttributeScoreFunc).toHaveBeenCalledTimes(gaitAttrsNoGaiting.length);
            expect(consoleWarnSpy).toHaveBeenCalledWith(expect.stringContaining('Conformation profiles not found'));
        });
        ...
        it('should use original generateAttributeScore if no dependency injected', () => {
            ...
            const realGenerateAttributeScoreSpy = jest.spyOn(ratingsEngine, 'generateAttributeScore');
            const profile = createProfile(false, true, true, true);
            ...
            expect(realGenerateAttributeScoreSpy.mock.calls.length).toBe(conformationAttributes.length + gaitAttrsNoGaiting.length);
    ...
    describe('generateAttributeScore (original implementation)', () => {
        ...
        it('should return DEFAULT_SCORE_FROM_SUT (50) and log warn if profile is undefined', () => {
            ...
            expect(consoleWarnSpy).toHaveBeenCalledWith(expect.stringContaining('Attribute profile is missing or invalid'), undefined);
        });
        ...
        it('should return DEFAULT_SCORE_FROM_SUT (50) and log warn if profile is missing mean', () => {
            ...
            const score = ratingsEngine.generateAttributeScore(profile);
            expect(score).toBe(DEFAULT_SCORE_FROM_SUT);
            expect(consoleWarnSpy).toHaveBeenCalledWith(expect.stringContaining('Attribute profile is missing or invalid'), profile);
        });
    ...
    describe('calculateFoalRatings', () => {
        ...
        const createParentData = (conformationOverrides = {}, gaitOverrides = {}, isGaited = true) => ({
            conformation: {
                head: 50, neck: 50, shoulders: 50, back: 50, hindquarters: 50, legs: 50, hooves: 50,
                ...conformationOverrides,
            },
            gaits: {
                walk: 50, trot: 50, canter: 50, gallop: 50, 
                gaiting: isGaited ? 50 : null,
                ...gaitOverrides,
            }
        });

        const createFoalBreedProfile = (isGaited = true, conformationProfiles = {}, gaitProfiles = {}) => ({
            is_gaited_breed: isGaited,
            conformation: {
                head: { mean: 50, std_dev: 5 }, neck: { mean: 50, std_dev: 5 }, /* ...defaults... */
                ...conformationProfiles,
            },
            gaits: {
                walk: { mean: 50, std_dev: 5 }, trot: { mean: 50, std_dev: 5 }, /* ...defaults... */
        ...
        it('should produce ratings within valid range for all attributes', () => {
            ...
            const { conformationRatings, gaitRatings } = ratingsEngine.calculateFoalRatings(sire, dam, foalProfile);
