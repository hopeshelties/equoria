...
const { selectWeightedRandom, determineStoreHorseTemperament, determineFoalTemperament, DEFAULT_TEMPERAMENT } = temperamentEngine;
...
describe('temperamentEngine', () => {
    ...
    describe('selectWeightedRandom', () => {
        ...
        it('should return DEFAULT_TEMPERAMENT if weightsObject is null, undefined, or (truly) empty object', () => {
            ...
            expect(consoleWarnSpy).toHaveBeenCalledWith(expect.stringContaining('Invalid or empty weightsObject'));
        ...
        it('should return the first valid temperament or DEFAULT_TEMPERAMENT as a final fallback', () => {
            ...
            expect(consoleWarnSpy).toHaveBeenCalledWith(expect.stringContaining('selectWeightedRandom did not select an item'));
        });
    });
    ...
    describe('determineStoreHorseTemperament', () => {
        ...
        it('should call injected selectWeightedRandom with breedTemperamentWeights', () => {
            ...
            const result = determineStoreHorseTemperament(breedWeights, { selectWeightedRandom: mockSelectWeightedRandomFunc });
        ...
        it('should return DEFAULT_TEMPERAMENT if breedTemperamentWeights is null, undefined, or empty', () => {
            expect(determineStoreHorseTemperament(null, {selectWeightedRandom: mockSelectWeightedRandomFunc})).toBe(DEFAULT_TEMPERAMENT);
            expect(consoleWarnSpy).toHaveBeenCalledWith(expect.stringContaining('Invalid or empty breedTemperamentWeights'));
            expect(mockSelectWeightedRandomFunc).not.toHaveBeenCalled(); // Should return before calling
            consoleWarnSpy.mockClear();
            
            expect(determineStoreHorseTemperament(undefined, {selectWeightedRandom: mockSelectWeightedRandomFunc})).toBe(DEFAULT_TEMPERAMENT);
            expect(consoleWarnSpy).toHaveBeenCalledWith(expect.stringContaining('Invalid or empty breedTemperamentWeights'));
            expect(mockSelectWeightedRandomFunc).not.toHaveBeenCalled();
            consoleWarnSpy.mockClear();

            expect(determineStoreHorseTemperament({}, {selectWeightedRandom: mockSelectWeightedRandomFunc})).toBe(DEFAULT_TEMPERAMENT);
            expect(consoleWarnSpy).toHaveBeenCalledWith(expect.stringContaining('Invalid or empty breedTemperamentWeights'));
            expect(mockSelectWeightedRandomFunc).not.toHaveBeenCalled();
        });
    ...
    describe('determineFoalTemperament', () => {
        ...
        it('should return DEFAULT_TEMPERAMENT if breedTemperamentWeights is null, undefined, or empty', () => {
            expect(determineFoalTemperament('Calm', 'Spirited', null, {selectWeightedRandom: mockSelectWeightedRandomFunc})).toBe(DEFAULT_TEMPERAMENT);
            expect(consoleWarnSpy).toHaveBeenCalledWith(expect.stringContaining('Invalid or empty breedTemperamentWeights for foal'));
            expect(mockSelectWeightedRandomFunc).not.toHaveBeenCalled();
        });
        ...
        it('should call injected selectWeightedRandom with weights adjusted by parental influence', () => {
            ...
            const result = determineFoalTemperament(sireT, damT, breedWeights, { selectWeightedRandom: mockSelectWeightedRandomFunc });
            ...
            expect(mockSelectWeightedRandomFunc).toHaveBeenCalledWith(expectedAdjustedWeights);
        ...
        it('should handle sire and dam having the same temperament (double bonus)', () => {
            ...
            const sireT = 'Calm';
            const damT = 'Calm';
            determineFoalTemperament(sireT, damT, breedWeights, { selectWeightedRandom: mockSelectWeightedRandomFunc });
            const expectedAdjustedWeights = {
        ...
        it('should log warnings if parental temperaments are not in breed weights and call with original weights (adjusted for present parents)', () => {
            ...
            expect(consoleWarnSpy).toHaveBeenCalledWith(expect.stringContaining(`Sire temperament '${sireT}' not in breed\'s base weights`));
            const expectedAdjustedWeights = {
        ...
        it('should use actual selectWeightedRandom logic with adjusted weights if no dependency injected', () => {
            ...
            mathRandomSpy.mockReturnValue(0.99); 
            expect(determineFoalTemperament(sireT, damT, breedWeights)).toBe('Spirited');
            mathRandomSpy.mockRestore();
        });
    });
}); 