...
const generateAttributeScore = (attributeProfile) => {
    if (!attributeProfile || typeof attributeProfile.mean !== 'number' || typeof attributeProfile.std_dev !== 'number') {
        console.warn('Attribute profile is missing or invalid, defaulting to 50.', attributeProfile);
...
const generateStoreHorseRatings = (breedRatingProfiles, _dependencies = {}) => {
    ...
    const _generateAttributeScore = _dependencies.generateAttributeScore || module.exports.generateAttributeScore;

    const conformationRatings = {};
    const gaitRatings = {};

    if (!breedRatingProfiles) {
        console.error('CRITICAL: breedRatingProfiles is undefined in generateStoreHorseRatings. Cannot generate ratings.');
        // Return default/empty ratings to prevent crashes, though this indicates a data setup issue.
        const defaultScore = 50;
        const confoAttrs = ['head', 'neck', 'shoulders', 'back', 'hindquarters', 'legs', 'hooves'];
    ...
    if (breedRatingProfiles.conformation) {
        for (const attr of conformationAttributes) {
            conformationRatings[attr] = _generateAttributeScore(breedRatingProfiles.conformation[attr]);
        }
    } else {
        console.warn('Conformation profiles not found in breedRatingProfiles, defaulting all to 50.');
    ...
    if (breedRatingProfiles.gaits) {
        for (const attr of gaitAttributes) {
            if (attr === 'gaiting') {
                if (breedRatingProfiles.is_gaited_breed && breedRatingProfiles.gaits[attr]) {
                    gaitRatings[attr] = _generateAttributeScore(breedRatingProfiles.gaits[attr]);
                } else {
        ...
        console.warn('Gait profiles not found in breedRatingProfiles, defaulting all to 50 (gaiting to null).');
...
const calculateFoalRatings = (sireCombinedRatings, damCombinedRatings, foalBreedRatingProfiles) => {
    const conformationAttributes = ['head', 'neck', 'shoulders', 'back', 'hindquarters', 'legs', 'hooves'];
    const gaitAttributes = ['walk', 'trot', 'canter', 'gallop', 'gaiting'];
    ...
    const calculateSingleFoalAttribute = (sireScore, damScore, attributeProfileForFoalBreed) => {
        // Handle cases where parent scores might be missing or null
        const sScore = (sireScore === undefined || sireScore === null) ? 50 : sireScore;
        const dScore = (damScore === undefined || damScore === null) ? 50 : damScore;

        const parentAverage = (sScore + dScore) / 2;
        
        let breedStdDev = 0;
        if (attributeProfileForFoalBreed && typeof attributeProfileForFoalBreed.std_dev === 'number') {
        ...
        const randomTweak = Math.floor(Math.random() * (RANDOM_TWEAK_RANGE * 2 + 1)) - RANDOM_TWEAK_RANGE;
    ...
    if (sireCombinedRatings && sireCombinedRatings.conformation && 
        damCombinedRatings && damCombinedRatings.conformation &&
        foalBreedRatingProfiles && foalBreedRatingProfiles.conformation) {
        for (const attr of conformationAttributes) {
        ...
        console.warn("[RatingsEngine] Missing parent conformation ratings or foal breed profile for conformation. Defaulting foal conformation ratings to 50.");
        conformationAttributes.forEach(attr => newConformationRatings[attr] = 50);
    }

    // Calculate Gait Ratings
    if (sireCombinedRatings && sireCombinedRatings.gaits &&
        damCombinedRatings && damCombinedRatings.gaits &&
        foalBreedRatingProfiles && foalBreedRatingProfiles.gaits) {
        for (const attr of gaitAttributes) {
            if (attr === 'gaiting') {
        ...
        console.warn("[RatingsEngine] Missing parent gait ratings or foal breed profile for gaits. Defaulting foal gait ratings.");
        gaitAttributes.forEach(attr => {
            if (attr === 'gaiting') {
                newGaitRatings[attr] = (foalBreedRatingProfiles && foalBreedRatingProfiles.is_gaited_breed) ? 50 : null;
            } else {
    ...
    return { conformationRatings: newConformationRatings, gaitRatings: newGaitRatings };
};
