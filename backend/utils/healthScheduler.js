...
const updateHorseHealth = async () => {
    ...
    try {
        ...
        const { rows: horses } = await client.query('SELECT id, health_status, last_vetted_date FROM horses');
        ...
        console.log(`Weekly horse health update completed. ${horsesUpdatedCount} horses updated.`);

    } catch (err) {
        console.error('[CRITICAL] Error during weekly horse health update process:', err.stack || err.message || err);
        try {
            await client.query('ROLLBACK');
            console.log('Transaction rolled back due to error in health update.');
        } catch (rollbackErr) {
            console.error('[CRITICAL-FATAL] Failed to ROLLBACK transaction after health update error:', rollbackErr.stack || rollbackErr.message || rollbackErr);
...
const startHealthScheduler = () => {
    ...
    if (healthUpdateJob) {
        console.log('Health scheduler potentially already initialized. Returning existing job or null.');
    ...
    console.log('Horse health scheduler started. Will run every Sunday at midnight.');
    return healthUpdateJob;
};
...
module.exports = { startHealthScheduler, updateHorseHealth, stopHealthScheduler, getHealthUpdateJob: () => healthUpdateJob }; 