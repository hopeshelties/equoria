...
const selectWeightedRandom = (weightsObject) => {
    if (!weightsObject || typeof weightsObject !== 'object' || Object.keys(weightsObject).length === 0) {
        console.warn('[TemperamentEngine] Invalid or empty weightsObject provided to selectWeightedRandom. Returning default.');
        return DEFAULT_TEMPERAMENT;
    }

    let totalWeight = 0;
    for (const temperament in weightsObject) {
        if (typeof weightsObject[temperament] === 'number' && weightsObject[temperament] > 0) {
    ...
    if (totalWeight === 0) {
        ...
        return keys.length > 0 ? keys[Math.floor(Math.random() * keys.length)] : DEFAULT_TEMPERAMENT;
    }
    ...
    console.warn('[TemperamentEngine] selectWeightedRandom did not select an item as expected. Returning default.');
    const keys = Object.keys(weightsObject).filter(k => typeof weightsObject[k] === 'number' && weightsObject[k] > 0);
 ...
 */
const determineStoreHorseTemperament = (breedTemperamentWeights, _dependencies = {}) => {
    const _selectWeightedRandom = _dependencies.selectWeightedRandom || module.exports.selectWeightedRandom; 
    if (!breedTemperamentWeights || typeof breedTemperamentWeights !== 'object' || Object.keys(breedTemperamentWeights).length === 0) {
        console.warn('[TemperamentEngine] Invalid or empty breedTemperamentWeights for store horse. Returning default temperament.');
        return DEFAULT_TEMPERAMENT;
    }
 ...
 */
const determineFoalTemperament = (sireTemperament, damTemperament, breedTemperamentWeights, _dependencies = {}) => {
    const _selectWeightedRandom = _dependencies.selectWeightedRandom || module.exports.selectWeightedRandom;

    if (!breedTemperamentWeights || typeof breedTemperamentWeights !== 'object' || Object.keys(breedTemperamentWeights).length === 0) {
        console.warn('[TemperamentEngine] Invalid or empty breedTemperamentWeights for foal. Returning default temperament.');
        return DEFAULT_TEMPERAMENT;
    }

    const adjustedWeights = { ...breedTemperamentWeights };

    if (sireTemperament && adjustedWeights.hasOwnProperty(sireTemperament)) {
        adjustedWeights[sireTemperament] = (adjustedWeights[sireTemperament] || 0) + PARENTAL_INFLUENCE_BONUS;
    } else if (sireTemperament) {
        console.warn(`[TemperamentEngine] Sire temperament '${sireTemperament}' not in breed\'s base weights. Not applying bonus.`);
    }

    if (damTemperament && adjustedWeights.hasOwnProperty(damTemperament)) {
        adjustedWeights[damTemperament] = (adjustedWeights[damTemperament] || 0) + PARENTAL_INFLUENCE_BONUS;
    } else if (damTemperament) {
        console.warn(`[TemperamentEngine] Dam temperament '${damTemperament}' not in breed\'s base weights. Not applying bonus.`);
    }
