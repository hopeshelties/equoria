...
async function createDatabases() {
  ...
  try {
    ...
    if (error.code === '42P04') {
      console.log('ℹ️ equoria already exists');
    } else {
      console.error('❌ Error creating equoria:', error.message);
    }
  }
  ...
  try {
    ...
    if (error.code === '42P04') {
      console.log('ℹ️ equoria_test already exists');
    } else {
      console.error('❌ Error creating equoria_test:', error.message);
    }
  } finally {
    await client.end();
  }
