...
const { determineStoreHorseTemperament, DEFAULT_TEMPERAMENT } = require('../utils/temperamentEngine'); // Import temperament engine
...
router.post('/purchase-horse', authMiddleware, async (req, res, next) => {
    ...
    if (!name || !breed_id || !sex) {
        return res.status(400).json({ message: 'Name, breed_id, and sex are required.' });
    }
    if (sex !== 'Stallion' && sex !== 'Mare') {
        return res.status(400).json({ message: 'Sex must be "Stallion" or "Mare" for store purchase.' });
    }
    ...
    try {
        ...
        const breedResult = await client.query('SELECT name, default_trait, breed_genetic_profile FROM breeds WHERE id = $1', [breed_id]);
        ...
        if (!breedGeneticProfile || !breedGeneticProfile.rating_profiles) {
            await client.query('ROLLBACK');
            console.error(`Breed ${breed.name} (ID: ${breed_id}) is missing rating_profiles in its breed_genetic_profile.`);
            return res.status(500).json({ message: `Configuration error: Breed ${breed.name} is missing rating profiles.` });
        }

        // Ensure temperament_weights exist
        if (!breedGeneticProfile.temperament_weights) {
            await client.query('ROLLBACK');
            console.error(`Breed ${breed.name} (ID: ${breed_id}) is missing temperament_weights in its breed_genetic_profile.`);
            return res.status(500).json({ message: `Configuration error: Breed ${breed.name} is missing temperament weights.` });
        }

        // 2. Determine Date of Birth (3 years old)
        const currentDate = new Date();
        const date_of_birth = new Date(currentDate.setFullYear(currentDate.getFullYear() - 3)).toISOString().split('T')[0];
        ...
        const { final_display_color, phenotypic_markings, determined_shade } = await determinePhenotype(fullGenotype, breedGeneticProfile, 3);
        ...
        const temperament = determineStoreHorseTemperament(breedGeneticProfile.temperament_weights);
        ...
        const newHorseParams = [
            name, owner_id, breed_id, sex, date_of_birth, final_display_color,
            fullGenotype, phenotypic_markings, precision, strength, speed,
            agility, endurance, personality, trait, determined_shade, temperament
        ];
        ...
        const { conformationRatings, gaitRatings } = generateStoreHorseRatings(breedGeneticProfile.rating_profiles);
        ...
        if (responseHorse && responseHorse.date_of_birth) {
            responseHorse.age_years = calculateAgeInYears(responseHorse.date_of_birth);
        }
        ...
        res.status(201).json({ message: 'Horse purchased successfully!', horse: responseHorse });
