const mockSendMail = jest.fn().mockResolvedValue({ messageId: 'mocked-message-id' }); // Define this at the very top and make it return a value
...
jest.mock('nodemailer', () => ({
    ...
    getTestMessageUrl: jest.fn().mockReturnValue('http://ethereal.example/preview/message')
}));
...
describe('Auth API Endpoints', () => {
    ...
    afterAll(async () => {
        if (pool && typeof pool.pool.end === 'function') { // Check if pool and pool.pool.end exist
            await pool.pool.end();
            console.log('Database pool closed.');
        } else {
            console.warn('Database pool or pool.end function not found, could not close pool.');
        }
        stopHealthScheduler(); // Stop the cron job
        if (expressServer && typeof expressServer.close === 'function') { // Close the main server if it was started
    ...
    describe('POST /api/users - User Signup', () => {
        ...
        it('should create a new user successfully and trigger a verification email', async () => {
            ...
            if (response.statusCode !== 201) {
                console.log('Successful Signup Failure - Response Body:', JSON.stringify(response.body, null, 2));
            }

            expect(response.statusCode).toBe(201);
            expect(response.body).toHaveProperty('message', 'Registration successful. Please check your email (or Ethereal preview link in console) to verify your account.');
            expect(response.body).toHaveProperty('userId');
            const userId = response.body.userId;

            // Check database
            const dbUserResult = await pool.query('SELECT * FROM users WHERE id = $1', [userId]);
            ...
            const isPasswordMatch = await bcrypt.compare(validUserData.password, dbUser.password_hash);
            expect(isPasswordMatch).toBe(true);

            // Check email was sent
            expect(mockSendMail).toHaveBeenCalledTimes(1);
            expect(mockSendMail.mock.calls[0][0].to).toBe(validUserData.email.toLowerCase());
            expect(mockSendMail.mock.calls[0][0].subject).toBe('Verify Your Email Address for HorseSim Game');
            expect(mockSendMail.mock.calls[0][0].html).toContain('verify-email/' + dbUser.email_verification_token);
        });
        ...
        it('should return 400 for missing username', async () => {
            ...
            console.log('Missing Username Response Body:', JSON.stringify(response.body, null, 2));
        ...
        it('should return 400 for invalid email format', async () => {
            ...
            expect(response.statusCode).toBe(400);
            expect(response.body.errors).toBeInstanceOf(Array);
            expect(response.body.errors.some(err => err.path === 'email' && err.msg === 'Please provide a valid email address.')).toBe(true);
        });
    ...
    describe('POST /api/auth/login - User Login', () => {
        ...
        beforeEach(async () => {
            ...
            await pool.query(
                ...
                [
                    signupData.username, signupData.email, hashedPassword, true, null, // Verified user
                    'unverifieduser', 'unverified@example.com', hashedPassword, false, 'some_token' // Unverified user
                ]
            );
        });
        ...
        it('should return 400 for missing email', async () => {
            ...
            const response = await request(app)
                ...
                .send(dataWithoutEmail);
            expect(response.statusCode).toBe(400);
            console.log('Login Missing Email Response Body:', JSON.stringify(response.body, null, 2));
            expect(response.body.errors).toBeInstanceOf(Array);
            expect(response.body.errors.some(err => err.path === 'email')).toBe(true);
        });
        ...
        it('should return 400 for missing password', async () => {
            ...
            const response = await request(app)
                ...
                .send(dataWithoutPassword);
            expect(response.statusCode).toBe(400);
            console.log('Login Missing Password Response Body:', JSON.stringify(response.body, null, 2));
            expect(response.body.errors).toBeInstanceOf(Array);
            expect(response.body.errors.some(err => err.path === 'password')).toBe(true);
        });
    });
    ...
    describe('GET /api/auth/verify-email/:token - Email Verification', () => {
        ...
        beforeEach(async () => {
            ...
            const result = await pool.query(
                ...
                [verificationUser.username, verificationUser.email, hashedPass, verificationUser.token]
            );
        ...
        it('should verify email successfully with a valid token', async () => {
            const response = await request(app)
                .get(`/api/auth/verify-email/${verificationUser.token}`);
            
            expect(response.statusCode).toBe(200);
            expect(response.body).toHaveProperty('message', 'Email verified successfully. You can now log in.');
        ...
        it('should return 404 if token path segment is effectively empty (e.g. whitespace string)', async () => {
            const response = await request(app)
                .get('/api/auth/verify-email/   '); // Test with whitespace token
