...
const { determineStoreHorseTemperament, DEFAULT_TEMPERAMENT } = require('../utils/temperamentEngine');
...
router.post('/', [authMiddleware, adminMiddleware], async (req, res, next) => {
  ...
  if (!name || !breed_id || !sex || !date_of_birth || !genotype) {
    return res.status(400).json({ message: 'Name, breed_id, sex, date_of_birth, and genotype are required fields.' });
  }

  // Validate sex
  const validSexes = ['Stallion', 'Mare', 'Colt', 'Filly'];
  if (!validSexes.includes(sex)) {
    return res.status(400).json({ message: `Sex must be one of: ${validSexes.join(', ')}` });
  }

  // Validate breed_id is a number
  if (isNaN(parseInt(breed_id))) {
    return res.status(400).json({ message: 'breed_id must be a valid number.' });
  }
  ...
  if (dobParts.length !== 3 || isNaN(parseInt(dobParts[0])) || isNaN(parseInt(dobParts[1])) || isNaN(parseInt(dobParts[2]))) {
    return res.status(400).json({ message: 'date_of_birth must be in MM-DD-YYYY format.' });
  }
  const month = parseInt(dobParts[0]);
  const day = parseInt(dobParts[1]);
  const year = parseInt(dobParts[2]);
  if (month < 1 || month > 12 || day < 1 || day > 31 || year < 1900 || year > new Date().getFullYear() + 5) { // Basic sanity check for date parts
      return res.status(400).json({ message: 'Invalid date parts for date_of_birth.' });
  }
  ...
  if (typeof genotype !== 'object' || genotype === null) {
    return res.status(400).json({ message: 'Genotype must be a valid JSON object.' });
  }

  // Validate health_status
  const validHealthStatuses = ['Excellent', 'Very Good', 'Good', 'Fair', 'Poor'];
  if (health_status !== undefined && !validHealthStatuses.includes(health_status)) {
    return res.status(400).json({ message: `Health status must be one of: ${validHealthStatuses.join(', ')}` });
  }
  ...
  if (last_vetted_date !== undefined) {
    ...
    if (lvdParts.length !== 3 || isNaN(parseInt(lvdParts[0])) || isNaN(parseInt(lvdParts[1])) || isNaN(parseInt(lvdParts[2]))) {
      return res.status(400).json({ message: 'last_vetted_date must be in YYYY-MM-DD format.' });
    }
    ...
    if (lvdMonth < 1 || lvdMonth > 12 || lvdDay < 1 || lvdDay > 31 || lvdYear < 1900 || lvdYear > new Date().getFullYear() + 5) {
        return res.status(400).json({ message: 'Invalid date parts for last_vetted_date.' });
    }
  ...
  const optionalIntegerFields = { owner_id, stable_id, sire_id, dam_id, precision, strength, speed, agility, endurance, personality, intelligence, stud_fee, sale_price }; 
  for (const field in optionalIntegerFields) {
      if (optionalIntegerFields[field] !== undefined && (isNaN(parseInt(optionalIntegerFields[field])) || parseInt(optionalIntegerFields[field]) < 0)) {
          return res.status(400).json({ message: `${field} must be a valid non-negative integer if provided.` });
      }
  }
  if (for_sale !== undefined && typeof for_sale !== 'boolean') {
      return res.status(400).json({ message: 'for_sale must be a boolean if provided.'});
  }
  ...
  try {
    ...
    const breedResult = await client.query('SELECT name, default_trait, breed_genetic_profile FROM breeds WHERE id = $1', [num_breed_id]);
    ...
    if (!breedGeneticProfile) {
        console.error(`FATAL: Breed ${num_breed_id} (${breedData.name}) is missing breed_genetic_profile.`);
        await client.query('ROLLBACK');
        return res.status(500).json({ message: `Configuration error: Breed ${breedData.name} is missing its genetic profile.`});
    }
    ...
    if (generateRatings && (!breedGeneticProfile.rating_profiles)) {
        console.error(`Configuration error: Breed ${breedData.name} (ID: ${num_breed_id}) is missing rating_profiles in its breed_genetic_profile. Cannot generate ratings.`);
        await client.query('ROLLBACK');
        return res.status(500).json({ message: `Configuration error: Breed ${breedData.name} is missing rating profiles required for generation.` });
    }
    ...
    const { final_display_color, determined_shade, phenotypic_markings } = determinePhenotype(genotype, breedGeneticProfile, ageInYears);
    ...
    if (!final_display_color) {
        ...
        console.error('determinePhenotype failed to return a final_display_color. Genotype:', genotype, 'Breed Profile:', breedGeneticProfile);
        await client.query('ROLLBACK');
        return res.status(400).json({ message: 'Could not determine horse phenotype based on provided genotype and breed profile. Check genotype structure and breed data.' });
    }
    ...
    if (adminTemperament) {
        if (!ALLOWED_TEMPERAMENTS.includes(adminTemperament)) {
            await client.query('ROLLBACK');
            return res.status(400).json({ message: `Invalid temperament value. Must be one of: ${ALLOWED_TEMPERAMENTS.join(', ')}` });
        }
        ...
        if (!breedGeneticProfile.temperament_weights) {
            await client.query('ROLLBACK');
            console.error(`Configuration error: Breed ${breedData.name} (ID: ${num_breed_id}) is missing temperament_weights. Cannot generate temperament.`);
            return res.status(500).json({ message: `Configuration error: Breed ${breedData.name} is missing temperament weights required for generation.` });
        }
        horseTemperament = determineStoreHorseTemperament(breedGeneticProfile.temperament_weights);
    ...
    `;
    const insertParams = [
        horseData.name, horseData.owner_id, horseData.stable_id, horseData.breed_id, horseData.sex, horseData.date_of_birth, horseData.genotype,
        horseData.final_display_color, horseData.shade, horseData.phenotypic_markings, horseData.sire_id, horseData.dam_id,
        horseData.precision, horseData.strength, horseData.speed, horseData.agility, horseData.endurance, horseData.personality, horseData.intelligence, horseData.trait,
        horseData.image_url, horseData.stud_status, horseData.stud_fee, horseData.for_sale, horseData.sale_price, horseData.health_status, horseData.last_vetted_date,
    ...
    if (adminConformationRatings && adminGaitRatings) {
        ...
        console.log('Generating ratings for horse ID:', newHorse.id, 'using breed profile.');
        const generated = generateStoreHorseRatings(breedGeneticProfile.rating_profiles);
    ...
    await client.query(confoQuery, [
        newHorse.id, conformationRatings.head, conformationRatings.neck, conformationRatings.shoulders, 
        conformationRatings.back, conformationRatings.hindquarters, conformationRatings.legs, conformationRatings.hooves
    ]);
    ...
    await client.query(gaitQuery, [
        newHorse.id, gaitRatings.walk, gaitRatings.trot, gaitRatings.canter, 
        gaitRatings.gallop, gaitRatings.gaiting
    ]);
    
    await client.query('COMMIT'); // Commit transaction

    // Fetch the full horse data including ratings to return to client
    const fullHorseDataResult = await db.query( // Use db.query here, not client.query, as transaction is committed
        `SELECT h.*, 
    ...
    if (err.constraint) { // Handle known DB constraint violations
        if (err.constraint === 'horses_sire_id_fkey' || err.constraint === 'horses_dam_id_fkey') {
            return res.status(400).json({ message: 'Invalid sire_id or dam_id. Referenced horse does not exist.'});
        }
        if (err.constraint === 'horses_breed_id_fkey') { // Should be caught by breed check earlier, but as a fallback
            return res.status(400).json({ message: 'Invalid breed_id. Referenced breed does not exist.'});
        }
        if (err.constraint === 'horses_owner_id_fkey') {
            return res.status(400).json({ message: 'Invalid owner_id. Referenced user does not exist.'});
        }
        if (err.constraint === 'horses_stable_id_fkey') {
            return res.status(400).json({ message: 'Invalid stable_id. Referenced stable does not exist.'});
        }
        // Add more specific constraint error handling if needed
        return res.status(400).json({ message: `Database constraint violation: ${err.constraint}` });
    }
...
router.get('/', authMiddleware, async (req, res, next) => {
  ...
  try {
    const userHorses = await db.query('SELECT * FROM horses WHERE owner_id = $1 ORDER BY created_at DESC', [owner_id]);
...
router.get('/:id', authMiddleware, async (req, res, next) => {
  ...
  if (isNaN(horseId)) {
    return res.status(400).json({ message: 'Horse ID must be a valid number.' });
  }
...
router.delete('/:id', [authMiddleware, adminMiddleware], async (req, res, next) => {
  ...
  try {
    ...
    const horseCheck = await db.query('SELECT id FROM horses WHERE id = $1', [horseId]);
    ...
    const deleteResult = await db.query('DELETE FROM horses WHERE id = $1 RETURNING id', [horseId]);

    if (deleteResult.rowCount === 0) {
        // Should have been caught by the check above, but as a safeguard
        return res.status(404).json({ message: 'Horse not found or already deleted.' });
    }

    res.json({ message: 'Horse deleted successfully.', deletedHorseId: horseId });

  } catch (err) {
    console.error(`Error deleting horse with ID ${horseId}:`, err.stack || err);
...
router.put('/:id', authMiddleware, async (req, res, next) => {
  ...
  try {
    ...
    const horseResult = await db.query('SELECT * FROM horses WHERE id = $1', [horseId]);
    if (horseResult.rows.length === 0) {
      return res.status(404).json({ message: 'Horse not found.' });
    }
    ...
    if (!isAdmin && !isOwner) {
      return res.status(403).json({ message: 'User not authorized to update this horse.' });
    }

    // 3. Prepare updates
    let newHorseData = { ...existingHorse }; // Start with existing data
    let needsPhenotypeRecalculation = false;
    let newAgeInYears = calculateAgeInYears(newHorseData.date_of_birth.toISOString().split('T')[0]); // Current age

    if (isAdmin) {
      // Admin can update more fields
      const {
        name, breed_id, sex, date_of_birth, genotype, owner_id, stable_id,
        sire_id, dam_id, precision, strength, speed, agility, endurance,
        personality, intelligence, trait, image_url, stud_status, stud_fee,
        for_sale, sale_price, health_status, last_vetted_date, temperament
      } = req.body;

      if (name !== undefined) newHorseData.name = name;
      if (owner_id !== undefined) newHorseData.owner_id = owner_id === null ? null : parseInt(owner_id);
      if (stable_id !== undefined) newHorseData.stable_id = stable_id === null ? null : parseInt(stable_id);
      if (sire_id !== undefined) newHorseData.sire_id = sire_id === null ? null : parseInt(sire_id);
      if (dam_id !== undefined) newHorseData.dam_id = dam_id === null ? null : parseInt(dam_id);
      ...
      if (personality !== undefined) newHorseData.personality = parseInt(personality);
      ...
      if (sale_price !== undefined) newHorseData.sale_price = parseInt(sale_price);
      ...
      if (last_vetted_date !== undefined) {
        ...
        if (lvdParts.length !== 3 || isNaN(parseInt(lvdParts[0])) || isNaN(parseInt(lvdParts[1])) || isNaN(parseInt(lvdParts[2]))) {
          return res.status(400).json({ message: 'last_vetted_date must be in YYYY-MM-DD format for admin update.' });
        }
        const lvdYear = parseInt(lvdParts[0]);
        const lvdMonth = parseInt(lvdParts[1]);
        const lvdDay = parseInt(lvdParts[2]);
      ...
      if (breed_id !== undefined && parseInt(breed_id) !== newHorseData.breed_id) {
        // Validate new breed_id exists
        const breedCheck = await db.query('SELECT id, breed_genetic_profile, default_trait FROM breeds WHERE id = $1', [parseInt(breed_id)]);
        if (breedCheck.rows.length === 0) return res.status(400).json({ message: 'Invalid new breed_id.'});
        newHorseData.breed_id = parseInt(breed_id);
        // If breed changes, trait might need to be re-evaluated if not explicitly set
        if (trait === undefined && newHorseData.trait !== breedCheck.rows[0].default_trait) {
      ...
      if (date_of_birth !== undefined) {
        ...
        if (dobParts.length !== 3 || isNaN(parseInt(dobParts[0])) || isNaN(parseInt(dobParts[1])) || isNaN(parseInt(dobParts[2]))) {
          return res.status(400).json({ message: 'date_of_birth must be in MM-DD-YYYY format for admin update.' });
        }
      ...
      if (sex !== undefined && ['Stallion', 'Mare', 'Colt', 'Filly'].includes(sex) && newHorseData.sex !== sex) {
      ...
      const currentAgeForSexCheck = calculateAgeInYears(newHorseData.date_of_birth.toISOString().split('T')[0]);
      ...
      if (genotype !== undefined) {
        if (typeof genotype !== 'object' || genotype === null) return res.status(400).json({ message: 'Genotype must be a valid JSON object for admin update.' });
        newHorseData.genotype = genotype;
        needsPhenotypeRecalculation = true;
      }

      if (temperament !== undefined) { // Admin updating temperament
      ...
      const { name, image_url, stud_status, stud_fee, for_sale, sale_price, temperament: ownerAttemptedTemperament } = req.body; // Check if owner tries to pass temperament
      let hasOwnerUpdates = false;

      if (ownerAttemptedTemperament !== undefined) {
          return res.status(403).json({ message: 'Owners are not allowed to update horse temperament.' });
      }

      if (name !== undefined && name !== newHorseData.name) { newHorseData.name = name; hasOwnerUpdates = true; }
      if (image_url !== undefined && image_url !== newHorseData.image_url) { 
        // Add a note about content moderation being a separate concern
        console.log('User updating image_url. Content moderation should be considered.');
        newHorseData.image_url = image_url; 
        hasOwnerUpdates = true; 
      }
      if (stud_status !== undefined && ['Not at Stud', 'Public Stud', 'Private Stud'].includes(stud_status) && stud_status !== newHorseData.stud_status) { 
        newHorseData.stud_status = stud_status; 
        hasOwnerUpdates = true; 
        if (stud_status === 'Not at Stud') newHorseData.stud_fee = 0; // Reset fee if not at stud
      }
      if (stud_fee !== undefined && !isNaN(parseInt(stud_fee)) && parseInt(stud_fee) >= 0 && parseInt(stud_fee) !== newHorseData.stud_fee) {
        if (newHorseData.stud_status === 'Not at Stud') return res.status(400).json({ message: 'Cannot set stud_fee if horse is Not at Stud.'});
        newHorseData.stud_fee = parseInt(stud_fee); 
        hasOwnerUpdates = true; 
      }
      if (for_sale !== undefined && typeof for_sale === 'boolean' && for_sale !== newHorseData.for_sale) { 
        newHorseData.for_sale = for_sale; 
        hasOwnerUpdates = true; 
        if (!for_sale) newHorseData.sale_price = 0; // Reset price if no longer for sale
      }
      if (sale_price !== undefined && !isNaN(parseInt(sale_price)) && parseInt(sale_price) >= 0 && parseInt(sale_price) !== newHorseData.sale_price) { 
        if (!newHorseData.for_sale) return res.status(400).json({ message: 'Cannot set sale_price if horse is not for sale.'});
        newHorseData.sale_price = parseInt(sale_price); 
        hasOwnerUpdates = true; 
      }
      
      if (Object.keys(req.body).some(key => !['name', 'image_url', 'stud_status', 'stud_fee', 'for_sale', 'sale_price'].includes(key))) {
          return res.status(403).json({ message: 'Owners can only update name, image_url, stud_status, stud_fee, for_sale, and sale_price.' });
      }
      if (!hasOwnerUpdates && Object.keys(req.body).length > 0) {
        return res.status(304).json({ message: 'No valid fields provided for update or values are the same.' }); // Not Modified
      }
    ...
    if (needsPhenotypeRecalculation) {
      const breedProfileResult = await db.query('SELECT breed_genetic_profile FROM breeds WHERE id = $1', [newHorseData.breed_id]);
      if (breedProfileResult.rows.length === 0 || !breedProfileResult.rows[0].breed_genetic_profile) {
        return res.status(500).json({ message: 'Could not retrieve breed genetic profile for phenotype recalculation.' });
      }
      const phenotypeResult = determinePhenotype(newHorseData.genotype, breedProfileResult.rows[0].breed_genetic_profile, newAgeInYears);
      newHorseData.final_display_color = phenotypeResult.final_display_color;
      newHorseData.shade = phenotypeResult.determined_shade;
      newHorseData.phenotypic_markings = phenotypeResult.phenotypic_markings || {};
    }

    // 4. Construct and execute update query
    // Ensure date_of_birth is in YYYY-MM-DD for the database
    const dobForDb = newHorseData.date_of_birth instanceof Date ? newHorseData.date_of_birth.toISOString().split('T')[0] : newHorseData.date_of_birth;
    ...
    `;
    const updateParams = [
      newHorseData.name, newHorseData.owner_id, newHorseData.stable_id, newHorseData.breed_id, 
      newHorseData.sex, dobForDb, newHorseData.genotype, 
      newHorseData.final_display_color, newHorseData.shade, newHorseData.phenotypic_markings, 
      newHorseData.sire_id, newHorseData.dam_id, newHorseData.precision, newHorseData.strength, 
      newHorseData.speed, newHorseData.agility, newHorseData.endurance, newHorseData.personality, 
      newHorseData.intelligence, newHorseData.trait, newHorseData.image_url, 
      newHorseData.stud_status, newHorseData.stud_fee, newHorseData.for_sale, newHorseData.sale_price,
      newHorseData.health_status, newHorseData.last_vetted_date, newHorseData.temperament,
      horseId
    ];
...
router.put('/:id/vet', authMiddleware, async (req, res, next) => {
  ...
  try {
    ...
    if (horseAndUserResult.rows.length === 0) {
      ...
      return res.status(404).json({ message: 'Horse not found or not owned by user.' }); 
    }
    ...
    if (horseData.game_currency < vettingCost) {
      await client.query('ROLLBACK');
      return res.status(400).json({ message: `Insufficient funds. Vetting costs ${vettingCost} game currency. You have ${horseData.game_currency}.` });
    }

    // 3. Deduct currency
    const newCurrency = horseData.game_currency - vettingCost;
    await client.query('UPDATE users SET game_currency = $1 WHERE id = $2', [newCurrency, userId]);
    ...
    if (updatedHorseResult.rows.length === 0) {
      ...
      await client.query('ROLLBACK');
      console.error(`Failed to update horse ${horseId} during vetting, though it was found initially and currency was deducted.`);
      return res.status(500).json({ message: 'Error vetting horse. Horse record may have changed unexpectedly. Transaction rolled back.'});
    }
