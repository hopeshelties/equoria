...
router.post('/visit', authMiddleware, async (req, res) => {
    ...
    try {
        ...
        } else { // 'basic' or any other unforeseen type defaults to basic
        ...
        if(updateResult.rowCount === 0) {
            await client.query('ROLLBACK');
            return res.status(500).json({ msg: 'Failed to update user bank details.' });
        }
        ...
        res.status(500).send('Server Error: Bank visit failed and was rolled back.');
    } finally {
