...
router.post('/', authMiddleware, async (req, res) => {
    ...
    if (!mare_id || !stallion_id) {
        return res.status(400).json({ msg: 'Mare ID and Stallion ID are required.' });
    }
    if (isNaN(parseInt(mare_id)) || isNaN(parseInt(stallion_id))) {
        return res.status(400).json({ msg: 'Mare ID and Stallion ID must be valid numbers.' });
    }
    ...
    try {
        ...
        const mareResult = await db.query('SELECT * FROM horses WHERE id = $1 AND owner_id = $2 AND sex = $3', [num_mare_id, mare_owner_id, 'Mare']);
        if (mareResult.rows.length === 0) {
            return res.status(404).json({ msg: 'Mare not found, not owned by you, or is not a Mare.' });
        }
        ...
        const stallionResult = await db.query('SELECT owner_id, stud_fee, stud_status FROM horses WHERE id = $1 AND sex = $2', [num_stallion_id, 'Stallion']);
        if (stallionResult.rows.length === 0) {
            return res.status(404).json({ msg: 'Stallion not found or is not a Stallion.' });
        }
        const stallion = stallionResult.rows[0];

        if (stallion.stud_status !== 'Private Stud') {
            return res.status(400).json({ msg: 'This stallion is not listed for Private Stud.' });
        }
        
        if (stallion.owner_id === mare_owner_id) {
            return res.status(400).json({ msg: 'You cannot send a breeding request to your own stallion through this system.' });
        }
        ...
        if (existingRequestResult.rows.length > 0) {
            return res.status(400).json({ msg: 'An active breeding request for this mare and stallion already exists.' });
        }
        ...
        `;
        const newRequestParams = [num_mare_id, num_stallion_id, mare_owner_id, stallion.owner_id, stud_fee_at_request, 'pending'];
...
router.get('/received', authMiddleware, async (req, res) => {
    ...
    try {
        ...
        console.error('Error fetching received breeding requests:', err.stack || err);
        res.status(500).send('Server Error');
    }
});
...
router.put('/:requestId/accept', authMiddleware, async (req, res) => {
    ...
    try {
        ...
        if (requestResult.rows.length === 0) {
            return res.status(404).json({ msg: 'Pending request not found or you are not authorized to accept it.' });
        }
...
router.put('/:requestId/reject', authMiddleware, async (req, res) => {
    ...
    try {
        ...
        if (requestResult.rows.length === 0) {
            return res.status(404).json({ msg: 'Pending request not found or you are not authorized to reject it.' });
        }
...
router.put('/:requestId/cancel', authMiddleware, async (req, res) => {
    ...
    try {
        ...
        if (requestResult.rows.length === 0) {
            return res.status(404).json({ msg: 'Pending request not found or you are not authorized to cancel it.' });
        }
