...
router.post('/', signupValidationRules, async (req, res, next) => { // Added validation rules as middleware
  ...
  try {
    ...
    const existingUserByUsername = await db.query('SELECT * FROM users WHERE username = $1', [username]);
    ...
    res.status(201).json({ 
      message: 'Registration successful. Please check your email (or Ethereal preview link in console) to verify your account.',
