...
const { calculateFoalGenetics, determinePhenotype } = require('../utils/geneticsEngine');
const { determineFoalTemperament, DEFAULT_TEMPERAMENT } = require('../utils/temperamentEngine');
...
const BREEDER_LEVEL_MULTIPLIERS = {
    ...
    4: 0.70,
    5: 0.75,
    default: 0.50 // For users who might not have a breeder_level or if it's outside 1-5
};
...
router.post('/breed-horse', authMiddleware, async (req, res) => {
    ...
    if (isNaN(parseInt(mare_id)) || isNaN(parseInt(stallion_id))) {
        return res.status(400).json({ msg: 'Mare ID and Stallion ID must be numbers.' });
    }

    if (mare_id === stallion_id) {
        return res.status(400).json({ msg: 'Mare and Stallion cannot be the same horse.' });
    }
    ...
    try {
        ...
        if (mareResult.rows.length === 0) {
            ...
            return res.status(404).json({ msg: 'Mare not found or not owned by user.' });
        }
        ...
        if (mare.sex !== 'Mare') {
            await client.query('ROLLBACK');
            return res.status(400).json({ msg: 'The first horse selected must be a Mare.' });
        }
        if (stallion.sex !== 'Stallion') {
            await client.query('ROLLBACK');
            return res.status(400).json({ msg: 'The second horse selected must be a Stallion.' });
        }
        if (mare.owner_id !== mare_owner_id) {
            await client.query('ROLLBACK');
            return res.status(403).json({ msg: 'User does not own the selected mare.' });
        }
        ...
        if (mare.last_bred_date) {
            ...
            if (lastBredDate > threeWeeksAgo) {
                await client.query('ROLLBACK');
                const timeRemaining = Math.ceil((lastBredDate.getTime() - threeWeeksAgo.getTime()) / (1000 * 60 * 60 * 24));
                return res.status(400).json({ msg: `Mare is still on breeding cooldown. Time remaining: ${timeRemaining} days.` });
            }
        }
        ...
        if (stallion.stud_status === 'Not at Stud') {
            await client.query('ROLLBACK');
            return res.status(400).json({ msg: 'Stallion is not currently available for stud.' });
        }
        ...
        if (stallion.stud_status === 'Private Stud') {
            ...
            if (requestResult.rows.length === 0) {
                await client.query('ROLLBACK');
                return res.status(403).json({ msg: 'No accepted breeding request found for this private stud stallion and mare.' });
            }
            breedingRequestId = requestResult.rows[0].id;
            stud_fee_to_pay = requestResult.rows[0].stud_fee_at_request !== null ? requestResult.rows[0].stud_fee_at_request : stallion.stud_fee;
        ...
        if (stud_fee_to_pay > 0) {
            if (mareOwnerCurrency < stud_fee_to_pay) {
                await client.query('ROLLBACK');
                return res.status(400).json({ msg: 'Mare owner has insufficient funds to pay the stud fee.' });
            }
            const updatedMareOwnerCurrency = mareOwnerCurrency - stud_fee_to_pay;
            await client.query('UPDATE users SET game_currency = $1 WHERE id = $2', [updatedMareOwnerCurrency, mare_owner_id]);
            mareOwnerCurrency = updatedMareOwnerCurrency;

            if (stallionOwnerUserId !== mare_owner_id) {
                const updatedStallionOwnerCurrency = stallionOwnerCurrency + stud_fee_to_pay;
                await client.query('UPDATE users SET game_currency = $1 WHERE id = $2', [updatedStallionOwnerCurrency, stallionOwnerUserId]);
            } else {
                const updatedStallionOwnerCurrencySelf = mareOwnerCurrency + stud_fee_to_pay;
                await client.query('UPDATE users SET game_currency = $1 WHERE id = $2', [updatedStallionOwnerCurrencySelf, stallionOwnerUserId]);
            }
        }
        ...
        if (mare.breed_id === stallion.breed_id) {
            ...
            if (crossbreedRuleResult.rows.length === 0) {
                await client.query('ROLLBACK');
                return res.status(400).json({ msg: 'This crossbreed is not allowed or no rule exists for it.' });
            }
        ...
        if (foalBreedDetailsResult.rows.length === 0) {
            await client.query('ROLLBACK');
            return res.status(500).json({ msg: 'Could not find breed details for the determined foal breed.' });
        }
        ...
        if (!foal_breed_profile) {
            await client.query('ROLLBACK');
            console.error(`Foal breed profile is null or undefined for breed_id: ${foal_breed_id}`);
            return res.status(500).json({ msg: 'Foal breed genetic profile is missing. Cannot proceed.' });
        }
        ...
        if (!foalTemperamentWeights || typeof foalTemperamentWeights !== 'object' || Object.keys(foalTemperamentWeights).length === 0) {
            console.warn(`[BreedingRoute] Foal breed (ID: ${foal_breed_id}) is missing temperament_weights. Foal will get default temperament.`);
        ...
        if (!sireGenotype || !damGenotype) {
            await client.query('ROLLBACK');
            return res.status(500).json({ msg: 'Sire or Dam genotype is missing. Cannot calculate foal genetics.' });
        }

        const foalGenotype = await calculateFoalGenetics(sireGenotype, damGenotype, foal_breed_profile);
        if (Object.keys(foalGenotype).length === 0) {
             await client.query('ROLLBACK');
             return res.status(500).json({ msg: 'Failed to calculate foal genotype. Ensure parent genotypes and breed profile are complete.' });
        }

        // 6. Determine Foal Phenotype using determinePhenotype(foalGenotype, foalBreedGeneticProfile).
        const foalPhenotypeResult = await determinePhenotype(foalGenotype, foal_breed_profile);
        const { final_display_color, phenotypic_markings, determined_shade: foal_shade } = foalPhenotypeResult;

        // 7. Calculate Foal Stats:
        const calculatedBreederMultiplier = BREEDER_LEVEL_MULTIPLIERS[breeder_level] || BREEDER_LEVEL_MULTIPLIERS.default;
        
        const foalStats = {
            precision:    Math.round((( (stallion.precision || 0)    + (mare.precision || 0)    ) / 2) * calculatedBreederMultiplier),
            strength:     Math.round((( (stallion.strength || 0) + (mare.strength || 0) ) / 2) * calculatedBreederMultiplier),
            speed:        Math.round((( (stallion.speed || 0) + (mare.speed || 0) ) / 2) * calculatedBreederMultiplier),
            agility:      Math.round((( (stallion.agility || 0) + (mare.agility || 0) ) / 2) * calculatedBreederMultiplier),
            endurance:    Math.round((( (stallion.endurance || 0) + (mare.endurance || 0) ) / 2) * calculatedBreederMultiplier),
            personality:  Math.round((( (stallion.personality || 0) + (mare.personality || 0) ) / 2) * calculatedBreederMultiplier),
            intelligence: Math.round((( (stallion.intelligence || 0) + (mare.intelligence || 0) ) / 2) * calculatedBreederMultiplier)
        };
        ...
        const foalTemperament = determineFoalTemperament(stallion.temperament, mare.temperament, foalTemperamentWeights);
        
        // 8c. Prepare Parent Ratings for Foal Calculation
        const sireCombinedRatings = {
            conformation: {
                head: stallion.cr_head, neck: stallion.cr_neck, shoulders: stallion.cr_shoulders, back: stallion.cr_back,
                hindquarters: stallion.cr_hindquarters, legs: stallion.cr_legs, hooves: stallion.cr_hooves
            },
            gaits: {
                walk: stallion.gr_walk, trot: stallion.gr_trot, canter: stallion.gr_canter, gallop: stallion.gr_gallop, 
                gaiting: stallion.gr_gaiting
            }
        };
        const damCombinedRatings = {
            conformation: {
                head: mare.cr_head, neck: mare.cr_neck, shoulders: mare.cr_shoulders, back: mare.cr_back,
                hindquarters: mare.cr_hindquarters, legs: mare.cr_legs, hooves: mare.cr_hooves
            },
            gaits: {
                walk: mare.gr_walk, trot: mare.gr_trot, canter: mare.gr_canter, gallop: mare.gr_gallop, 
        ...
        if (!foal_breed_profile.rating_profiles) {
            console.warn(`[BreedingRoute] Foal breed (ID: ${foal_breed_id}) is missing rating_profiles. Foal will get default ratings (50s).`);
            // calculateFoalRatings will handle foalBreedRatingProfiles being undefined and default ratings
        }
        const calculatedRatings = calculateFoalRatings(sireCombinedRatings, damCombinedRatings, foal_breed_profile.rating_profiles);
        ...
        `;
        const foalValues = [
            foalName, mare_owner_id, null, foal_breed_id, foalSex, foalDob,
            final_display_color, foalGenotype, phenotypic_markings, foal_shade,
            stallion.id, mare.id,
            foalStats.precision, foalStats.strength, foalStats.speed, foalStats.agility, 
            foalStats.endurance, foalStats.personality, foalStats.intelligence,
            foalTrait, foalTemperament, null
        ];
        ...
        await client.query(confoQuery, [
            foalCreated.id, foalConformationRatings.head, foalConformationRatings.neck, foalConformationRatings.shoulders, 
            foalConformationRatings.back, foalConformationRatings.hindquarters, foalConformationRatings.legs, foalConformationRatings.hooves
        ]);
        ...
        await client.query(gaitQuery, [
            foalCreated.id, foalGaitRatings.walk, foalGaitRatings.trot, foalGaitRatings.canter, 
            foalGaitRatings.gallop, foalGaitRatings.gaiting
        ]);
        ...
        if (updateMareResult.rows.length === 0) {
            await client.query('ROLLBACK');
            console.error("CRITICAL: Foal created but mare's last_bred_date not updated for mare_id:", mare.id);
            return res.status(500).json({ msg: 'Failed to update mare last_bred_date. Foal creation rolled back.' });
        }
        const mareUpdated = updateMareResult.rows[0];

        // 11. Update breeding_request status if applicable
        if (breedingRequestId) {
            await client.query('UPDATE breeding_requests SET status = $1, updated_at = NOW() WHERE id = $2', ['completed', breedingRequestId]);
        }

        await client.query('COMMIT'); // Commit transaction
        
        // Fetch the full foal data including ratings to return to client
        const fullFoalDataResult = await db.query( // Use db.query, transaction is committed
            `SELECT h.*, 
        ...
        res.status(200).json({ msg: 'Foal created successfully. Stud fee processed.', foal: foalDataForResponse, mare: mareUpdated });
        ...
        res.status(500).send('Server Error: Breeding process failed and was rolled back.');
    } finally {
