...
router.post('/enter', auth, [
  ...
  try {
    ...
    const errors = validationResult(req);
    if (!errors.isEmpty()) {
      logger.warn(`[competitionRoutes.POST /enter] Validation errors: ${JSON.stringify(errors.array())}`);
      return res.status(400).json({
        success: false,
        message: 'Validation failed',
        errors: errors.array()
    ...
    logger.info(`[competitionRoutes.POST /enter] User ${userId} entering horse ${horseId} in show ${showId}`);
    ...
    logger.info(`[competitionRoutes.POST /enter] Successfully entered horse ${horseId} in show ${showId}`);
...
router.post('/execute', auth, [
  ...
  try {
    ...
    const errors = validationResult(req);
    if (!errors.isEmpty()) {
      logger.warn(`[competitionRoutes.POST /execute] Validation errors: ${JSON.stringify(errors.array())}`);
      return res.status(400).json({
        success: false,
        message: 'Validation failed',
        errors: errors.array()
    ...
    logger.info(`[competitionRoutes.POST /execute] User ${userId} executing competition for show ${showId}`);
    ...
    logger.info(`[competitionRoutes.POST /execute] Executing competition with ${competitionEntries.length} entries`);
    ...
    logger.info(`[competitionRoutes.POST /execute] Competition executed successfully for show ${showId}`);
 ...
 */
router.get('/eligibility/:horseId/:discipline', auth, [
  param('horseId')
    .isInt({ min: 1 })
    .withMessage('Horse ID must be a positive integer'),
  param('discipline')
  ...
  try {
    ...
    const errors = validationResult(req);
    if (!errors.isEmpty()) {
      logger.warn(`[competitionRoutes.GET /eligibility] Validation errors: ${JSON.stringify(errors.array())}`);
      return res.status(400).json({
        success: false,
        message: 'Validation failed',
        errors: errors.array()
    ...
    logger.info(`[competitionRoutes.GET /eligibility] Checking eligibility for horse ${horseId} in ${discipline}`);
    ...
    logger.info(`[competitionRoutes.GET /eligibility] Horse ${horseId} eligibility checked for ${discipline}`);
