...
describe('horseSeed', () => {
  ...
  describe('checkHorseExists', () => {
    ...
    it('should return false and log warning on database error', async() => {
      ...
      expect(result).toBe(false);
      expect(mockLogger.warn).toHaveBeenCalledWith('[seed] Failed to check if horse "Error Horse" exists: Database error');
    });
  });
  ...
  describe('findOrCreateBreed', () => {
    ...
    it('should return null for undefined breed name', async() => {
      ...
      expect(mockLogger.warn).toHaveBeenCalledWith('[seed] Breed name is undefined or null. Skipping breed creation/connection.');
    ...
    it('should throw error if database operation fails', async() => {
      ...
      expect(mockLogger.error).toHaveBeenCalledWith('[seed] Failed to find or create breed "Test Breed": Database connection failed');
    });
  });
  ...
  describe('ensureReferencedRecordsExist', () => {
    ...
    it('should handle errors gracefully when user creation fails', async() => {
      ...
      expect(mockLogger.warn).toHaveBeenCalledWith('[seed] Could not ensure User ID 1. Error: Database connection failed');
      expect(mockLogger.warn).toHaveBeenCalledWith('[seed] Could not ensure User ID 2. Error: Database connection failed');
      expect(mockLogger.info).toHaveBeenCalledWith('[seed] Ensured Stable ID 1 exists.');
      expect(mockLogger.info).toHaveBeenCalledWith('[seed] Ensured Stable ID 2 exists.');
    });
  ...
  describe('seedHorses', () => {
    ...
    it('should create breeds and horses for the provided user', async() => {
      ...
      expect(mockLogger.info).toHaveBeenCalledWith(`Created horse: ${mockHorseLightning.name} for user ID: ${mockUser.id}`);
      expect(mockLogger.info).toHaveBeenCalledWith(`Created horse: ${mockHorseDesertRose.name} for user ID: ${mockUser.id}`);
    });
    ...
    it('should log a warning if a horse is skipped due to missing breedId', async() => {
      ...
      expect(mockLogger.warn).toHaveBeenCalledWith(expect.stringContaining('Skipping horse Lightning Bolt due to missing breedId.'));
    ...
    it('should log an error if horse creation fails', async() => {
      ...
      expect(mockLogger.error).toHaveBeenCalledWith(expect.stringContaining('Error seeding horse Lightning Bolt: Failed to create horse'));
  ...
  describe('main execution (integration)', () => {
    it('should call ensureReferencedRecordsExist and seedHorses with sample data', async() => {
      const {
        default: actualMain
      } = await import(join(__dirname, './horseSeed.js'));
